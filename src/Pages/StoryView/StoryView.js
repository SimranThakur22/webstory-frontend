...
useEffect(() => {
    const fetchData = async () => {
        try {
            const allStoriesResponse = await axios.get('http://localhost:4000/allstories');
            setAllStory(allStoriesResponse.data.data.stories);

            // console.log(allStoriesResponse.data.data.stories);
			console.log(allstory);

            const story = allStoriesResponse.data.data.stories.find(item => item.id === Number(id));
            console.log(story.category, story.creation_time);

            const insideStoryResponse = await axios.get(`http://localhost:4000/getsamecategorystory?category=${story.category}&creation_time=${story.creation_time}`);
    ...
    {insidestory.map((src, index) => (
      ...
      <div
       ...
       >
        ...
        <FaBookmark size={30} style={bookmarkitem[src.id]===1 ? {color: 'blue', zIndex: 1000,cursor:'pointer'} : {color: 'white', zIndex: 1000,cursor:'pointer'}}onClick={()=>{
            console.log(src.id);
            console.log(bookmarkitem[src.id]);
            if(localStorage.getItem("auth-token")){
            if(bookmarkitem[src.id]===0){
            addbookmark(src.id);
            console.log("Bookmarkitem with ID " + src.id + " is  set to 1");
          }
          else {
            removebookmark(src.id);
            console.log("Bookmarkitem with ID " + src.id + " is  set to 0");
        }
      }else{
        window.location.replace("/login");
          ...
          <FaHeart size={30} style={likestoryitem[src.id]===1 ? {color: 'red', zIndex: 1000,cursor:'pointer'} : {color: 'white', zIndex: 1000,cursor:'pointer'}}onClick={()=>{
            console.log(src.id);
            ...
            console.log("likestoryitem with ID " + src.id + " is  set to 1");
          }
          else {
            ...
            const updatecount=[...likecount];
             updatecount[index]-=1;
             setlikecount(updatecount);
            console.log("likestoryitem with ID " + src.id + " is  set to 0");
        }
      }else{
        window.location.replace("/login");
      }
      ...
      {toast?<div className='toast'>Link copied to clipboard!!</div>:<></>}
        <div className='actions-2'>
          ...
          <MdClose size={28} onClick={()=>{navigate("/")}} style={{"color":"white","zIndex":'10000',cursor:'pointer'}}src={cross}/>
          <img  onClick={() => settoast(true)} style={{"color":"white","zIndex":'9000',cursor:'pointer'}} src={send}></img>
        </div>
      </div>
    ))}
