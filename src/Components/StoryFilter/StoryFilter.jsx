...
const StoryFilter = (props) => {
  useEffect(() => {
    try {
      const cat = props.category == "health" ? "health and fitness" : props.category
      axios
        .get(
          `https://webstory-backend2.onrender.com/top5stories?category=${cat}`
        )
        .then((res) => {
          // settraveldata(res.data.data.topstory_ids);
          console.log("data for -> ", res);
        });
    } catch (e) {
      console.log("error", e);
    }
